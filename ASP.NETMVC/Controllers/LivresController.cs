...
namespace ASP.NETMVC.Controllers
{
    public class LivresController : Controller
    {
        ...
        // GET: Livres
        public ActionResult Index(string sortOrder ,string  genre, string annee,string auteur)
        {
            //viewbag pour le trie
            ViewBag.AuteurTrie = String.IsNullOrEmpty(sortOrder) ? "Adesc" : "";
            ViewBag.AnneeTrie = sortOrder == "Date" ? "Date_desc" : "Date";
            ...
            { 
                livres = livres.Where(s => s.ANNEE_EDITION.Value.Year == int.Parse(annee));
            }
            //filtrage genre
            ...
            //condition pour trie le livre
            switch (sortOrder)
            {
                case "Adesc":
                    livres = livres.OrderByDescending(s => s.AUTEURS.NOM_AUTEUR);
                    break;
                case "Date":
                    livres = livres.OrderBy(s => s.ANNEE_EDITION);
                    break;
                case "Date_desc":
                    livres = livres.OrderByDescending(s => s.ANNEE_EDITION);
                    break;
                default:
                    livres = livres.OrderBy(s => s.AUTEURS.NOM_AUTEUR);
                    break;
            }
